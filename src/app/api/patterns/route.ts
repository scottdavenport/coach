...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `🔍 Pattern recognition requested for user ${user.id} over last ${daysBack} days`
    );
    ...
    const userPatterns = await patternService.analyzeUserPatterns(
      user.id,
      daysBack
    );
    ...
    console.log(
      `📊 Found ${userPatterns.conversationPatterns.length} conversation patterns`
    );
    console.log(
      `📊 Found ${userPatterns.topicPreferences.length} topic preferences`
    );
    console.log(
      `📊 Found ${userPatterns.activityPatterns.length} activity patterns`
    );
