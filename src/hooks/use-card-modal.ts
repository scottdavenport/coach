  ...
  const fetchAvailableDates = useCallback(async () => {
    try {
      ...
      if (dates.length > 0 && !selectedDate) {
        ...
        console.log(
          'No data exists, setting selectedDate to today:',
          todayString
        );
      ...
      try {
        ...
        if (metrics) {
            ...
            if (
              ...
              (metric.confidence === existingMetric.confidence &&
                new Date(metric.created_at) >
                  new Date(existingMetric.created_at))
            ) {
          ...
          uniqueMetrics.forEach(metric => {
            const categoryName =
              metric.standard_metrics?.metric_categories?.name || 'unknown';
            const categoryDisplayName =
              metric.standard_metrics?.metric_categories?.display_name ||
              'Unknown';
