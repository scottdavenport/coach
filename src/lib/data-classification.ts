export interface DataClassification {
  ...
  category: 'health' | 'fitness' | 'wellness' | 'lifestyle' | 'biometric' | 'sleep' | 'activity'
...
export async function classifyData(key: string, value: any): Promise<DataClassification> {
  ...
  if (valueType === 'number' || valueType === 'boolean') {
    if (lowerKey.includes('weight') || lowerKey.includes('heart') || lowerKey.includes('rate')) {
      return {
        type: 'metric',
        category: 'biometric',
        displayType: 'card',
    ...
    if (lowerKey.includes('sleep') || lowerKey.includes('mood') || lowerKey.includes('energy')) {
      return {
        type: 'metric',
        category: 'wellness',
        displayType: 'card',
    ...
    if (lowerKey.includes('workout') || lowerKey.includes('exercise') || lowerKey.includes('activity')) {
      return {
        type: 'metric',
        category: 'fitness',
        displayType: 'card',
  ...
  if (valueType === 'string' || Array.isArray(value)) {
    if (lowerKey.includes('tip') || lowerKey.includes('advice') || lowerKey.includes('suggestion')) {
      return {
        type: 'journal',
        category: 'lifestyle',
        displayType: 'journal',
...
export function separateDataByType(data: Record<string, any>): {
  ...
  // Sort by priority
  dailyCard.sort((a, b) => a.classification.priority - b.classification.priority)
...
function classifyDataSync(key: string, value: any): DataClassification {
  ...
  // Metric classification (numeric/boolean values)
  if (valueType === 'number' || valueType === 'boolean') {
    if (lowerKey.includes('weight') || lowerKey.includes('heart') || lowerKey.includes('rate') || lowerKey.includes('hr')) {
      return {
        type: 'metric',
        category: 'biometric',
        displayType: 'card',
    ...
    if (lowerKey.includes('sleep') || lowerKey.includes('mood') || lowerKey.includes('energy') || lowerKey.includes('stress') || lowerKey.includes('readiness')) {
      return {
        type: 'metric',
        category: 'wellness',
        displayType: 'card',
