...
class OcrTrainingDataCollector {
  ...
  async submitTrainingData(
    data: Omit<
      OcrTrainingData,
      'id' | 'timestamp' | 'isAnonymized' | 'approvedForTraining'
    >
  ) {
    try {
    ...
    try {
      ...
      return {
        ...
        averageProcessingTime:
          data.reduce((sum, item) => sum + item.processingTime, 0) /
          data.length,
      };
    } catch (error) {
  ...
  detectAppType(ocrText: string): string {
    ...
    if (text.includes('apple health') || text.includes('health app'))
      return 'apple_health';
  ...
  detectScreenshotType(ocrText: string): string {
    ...
    if (
      ...
      return 'activity';
    if (text.includes('readiness') || text.includes('recovery'))
      return 'readiness';
    if (text.includes('heart rate') || text.includes('hrv'))
      return 'heart_rate';
