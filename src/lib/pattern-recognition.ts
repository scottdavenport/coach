...
export class PatternRecognitionService {
  ...
  async analyzeUserPatterns(
    ...
    try {
      console.log(
        `🔍 Analyzing patterns for user ${userId} over last ${daysBack} days`
      );
      ...
      console.log(
        `📊 Found ${insights.length} conversation insights to analyze`
      );

      // Analyze different types of patterns
      const conversationPatterns =
        await this.analyzeConversationPatterns(insights);
   ...
   */
  private async analyzeTopicPreferences(
    insights: any[]
  ...
  private extractWakeTime(message: string): string {
    const timeMatch = message.match(
      /(?:woke|wake|up)\s*(?:at|around)?\s*(\d{1,2}):?(\d{2})?\s*(?:pm|am)/i
    );
    if (timeMatch) {
  ...
  private extractSleepFactors(message: string): string[] {
    ...
    if (message.includes('stress') || message.includes('anxious'))
      factors.push('stress');
    if (message.includes('caffeine') || message.includes('coffee'))
      factors.push('caffeine');
