...
export function ChatInterface({
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (
        data.parsedData &&
        (data.parsedData.has_health_data ||
          data.parsedData.has_activity_data ||
          data.parsedData.has_mood_data ||
          data.parsedData.has_nutrition_data ||
          data.parsedData.has_sleep_data ||
          data.parsedData.has_workout_data)
      ) {
  ...
  const handleFilesWithContext = useCallback(async () => {
    ...
    try {
      ...
      const fileSummary = fileManager.files
        .map(
          file =>
      ...
      for (const imageFile of imageFiles) {
        if (imageFile.fileUrl) {
          try {
            console.log(
              '🚀 Calling OCR Edge Function with:',
              imageFile.fileUrl
            );
      ...
      const aiResponse = await sendToAIWithMultiFileData(
        allProcessedContent,
        contextText
      );
  ...
  const handleOcrCorrection = async (correction: any, userMessage: string) => {
    ...
    try {
      ...
      const correctedData = extractCorrections(
        userMessage,
      ...
      const aiResponse = await sendCorrectionToAI(
        correction.originalData,
        correctedData,
        userMessage
      );
  ...
  const extractCorrections = (message: string, originalData: any) => {
    ...
    if (
      lowerMessage.includes('there is no sleep score') ||
      lowerMessage.includes('no sleep score')
    ) {
    ...
    const sleepMatch = lowerMessage.match(
      /(?:sleep|sleep score) (?:was|is|should be|is actually) (\d+)/i
    );
    if (sleepMatch) {
    ...
    const readinessMatch = lowerMessage.match(
      /(?:readiness|readiness score) (?:was|is|should be|is actually) (\d+)/i
    );
    if (readinessMatch) {
    ...
    const activityMatch = lowerMessage.match(
      /(?:activity|activity score) (?:was|is|should be|is actually) (\d+)/i
    );
    if (activityMatch) {
    ...
    const heartRateMatch = lowerMessage.match(
      /(?:heart rate|hr) (?:was|is|should be|is actually) (\d+)/i
    );
    if (heartRateMatch) {
    ...
    if (
      lowerMessage.includes('not my resting heart rate') ||
      lowerMessage.includes('not resting heart rate')
    ) {
    ...
    if (
      lowerMessage.includes('latest heart rate') &&
      lowerMessage.includes('not') &&
      lowerMessage.includes('resting')
    ) {
      correctedData.restingHeartRate = null;
      correctedData.resting_heart_rate = null;
      correctedData.heartRate = null;
    ...
    const glucoseMatch = lowerMessage.match(
      /(?:glucose|blood sugar) (?:was|is|should be|is actually) (\d+)/i
    );
    if (glucoseMatch) {
    ...
    const hrvMatch = lowerMessage.match(
      /(?:heart rate variability|hrv) (?:was|is|should be|is actually) (\d+)/i
    );
    if (hrvMatch) {
    ...
    const bodyTempMatch = lowerMessage.match(
      /(?:body temperature|temperature) (?:was|is|should be|is actually) ([±]?\d+(?:\.\d+)?)/i
    );
    if (bodyTempMatch) {
    ...
    const respRateMatch = lowerMessage.match(
      /(?:respiratory rate|breathing rate) (?:was|is|should be|is actually) (\d+(?:\.\d+)?)/i
    );
    if (respRateMatch) {
    ...
    const deepSleepMatch = lowerMessage.match(
      /(?:deep sleep|deep) (?:was|is|should be|is actually) (\d+)h (\d+)m/i
    );
    if (deepSleepMatch) {
    ...
    const totalSleepMatch = lowerMessage.match(
      /(?:total sleep|i slept for|i got) (?:was|is|should be|is actually) (\d+)h (\d+)m/i
    );
    if (totalSleepMatch) {
    ...
    if (
      lowerMessage.includes("didn't sleep very well") ||
      lowerMessage.includes('slept poorly') ||
      lowerMessage.includes('sleep was bad')
    ) {
    ...
    if (
      lowerMessage.includes('slept great') ||
      lowerMessage.includes('slept well') ||
      lowerMessage.includes('sleep was good')
    ) {
    ...
    const changedKeys = Object.keys(correctedData).filter(
      key => correctedData[key] !== originalData[key]
    );
  ...
  const sendErrorToAI = async (errorType: string) => {
    ...
    const errorMessages = {
      'connection issue':
        "I'm having trouble connecting right now. Could you try sending your message again? Sometimes a quick refresh helps clear up connection issues.",
      'OCR processing failure':
        'I had trouble processing that file. Could you try uploading it again? Make sure the image is clear and readable.',
      'file processing issue':
        "I'm having trouble with that file right now. Could you try again in a moment?",
      'data update issue':
        'I had trouble updating that data. Could you try again?',
      'correction response generation':
        "I've updated your data, but I'm having trouble generating a response right now. The changes have been saved though!",
      'OCR response generation':
        "I processed your health data, but I'm having trouble responding right now. Your data has been saved though!",
    };
  ...
  const sendCorrectionToAI = async (
    ...
    try {
      ...
      if (originalData.readiness_score !== correctedData.readiness_score) {
        changes.push(
          `readiness score: ${originalData.readiness_score || 'not detected'} → ${correctedData.readiness_score}`
        );
      }
      if (originalData.sleepScore !== correctedData.sleepScore) {
        changes.push(
          `sleep score: ${originalData.sleepScore || 'not detected'} → ${correctedData.sleepScore}`
        );
      }
      if (originalData.restingHeartRate !== correctedData.restingHeartRate) {
        changes.push(
          `resting heart rate: ${originalData.restingHeartRate || 'not detected'} → ${correctedData.restingHeartRate} bpm`
        );
      }
      if (
        originalData.resting_heart_rate !== correctedData.resting_heart_rate
      ) {
        changes.push(
          `resting heart rate: ${originalData.resting_heart_rate || 'not detected'} → ${correctedData.resting_heart_rate} bpm`
        );
      }
      if (
        originalData.heartRateVariability !== correctedData.heartRateVariability
      ) {
        changes.push(
          `HRV: ${originalData.heartRateVariability || 'not detected'} → ${correctedData.heartRateVariability} ms`
        );
      }
      if (originalData.bodyTemperature !== correctedData.bodyTemperature) {
        changes.push(
          `body temperature: ${originalData.bodyTemperature || 'not detected'} → ${correctedData.bodyTemperature}°F`
        );
      }
      if (originalData.respiratoryRate !== correctedData.respiratoryRate) {
        changes.push(
          `respiratory rate: ${originalData.respiratoryRate || 'not detected'} → ${correctedData.respiratoryRate}/min`
        );
      }
      if (originalData.totalSleep !== correctedData.totalSleep) {
        const originalFormatted = originalData.totalSleep
          ? `${Math.floor(originalData.totalSleep / 60)}h ${originalData.totalSleep % 60}m`
          : 'not detected';
        const correctedFormatted = correctedData.totalSleep
          ? `${Math.floor(correctedData.totalSleep / 60)}h ${correctedData.totalSleep % 60}m`
          : 'removed';
        changes.push(
          `total sleep: ${originalFormatted} → ${correctedFormatted}`
        );
      }
      if (originalData.timeInBed !== correctedData.timeInBed) {
        const originalFormatted = originalData.timeInBed
          ? `${Math.floor(originalData.timeInBed / 60)}h ${originalData.timeInBed % 60}m`
          : 'not detected';
        const correctedFormatted = correctedData.timeInBed
          ? `${Math.floor(correctedData.timeInBed / 60)}h ${correctedData.timeInBed % 60}m`
          : 'removed';
        changes.push(
          `time in bed: ${originalFormatted} → ${correctedFormatted}`
        );
      }
      if (originalData.sleepEfficiency !== correctedData.sleepEfficiency) {
        changes.push(
          `sleep efficiency: ${originalData.sleepEfficiency || 'not detected'} → ${correctedData.sleepEfficiency}%`
        );
      }
      if (originalData.remSleep !== correctedData.remSleep) {
        const originalFormatted = originalData.remSleep
          ? `${Math.floor(originalData.remSleep / 60)}h ${originalData.remSleep % 60}m`
          : 'not detected';
        const correctedFormatted = correctedData.remSleep
          ? `${Math.floor(correctedData.remSleep / 60)}h ${correctedData.remSleep % 60}m`
          : 'removed';
        changes.push(`REM sleep: ${originalFormatted} → ${correctedFormatted}`);
      }
      if (originalData.deepSleep !== correctedData.deepSleep) {
        const originalFormatted = originalData.deepSleep
          ? `${Math.floor(originalData.deepSleep / 60)}h ${originalData.deepSleep % 60}m`
          : 'not detected';
        const correctedFormatted = correctedData.deepSleep
          ? `${Math.floor(correctedData.deepSleep / 60)}h ${correctedData.deepSleep % 60}m`
          : 'removed';
        changes.push(
          `deep sleep: ${originalFormatted} → ${correctedFormatted}`
        );
      }
      if (originalData.steps !== correctedData.steps) {
        changes.push(
          `steps: ${originalData.steps || 'not detected'} → ${correctedData.steps}`
        );
      }
      if (originalData.calories !== correctedData.calories) {
        changes.push(
          `calories: ${originalData.calories || 'not detected'} → ${correctedData.calories}`
        );
      }
      if (originalData.glucose !== correctedData.glucose) {
        changes.push(
          `glucose: ${originalData.glucose || 'not detected'} → ${correctedData.glucose} mg/dL`
        );
      }

      const changesSummary =
        changes.length > 0 ? changes.join(', ') : 'some corrections';
      ...
      const data = await response.json();
      return (
        data.message || (await sendErrorToAI('correction response generation'))
      );
    } catch (error) {
  ...
  const sendToAIWithMultiFileData = async (
        ...
        switch (type) {
          ...
          case 'documents':
            fileInputRef.current.accept =
              '.pdf,.doc,.docx,.txt,.md,.csv,.xlsx,.ods,.pptx';
            break;
          default:
  ...
  const handleEmergencyMessage = useCallback(
  ...
  const callOcrFunction = async (imageUrl: string, userId: string) => {
    try {
      ...
      const response = await fetch(
        'https://uqzgbvcrnxdzfgmkkoxb.supabase.co/functions/v1/ocr',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `OCR request failed: ${response.status} ${response.statusText} - ${errorText}`
        );
      }
