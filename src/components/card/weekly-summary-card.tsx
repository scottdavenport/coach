...
export function WeeklySummaryCard({
  ...
  const { summary, loading, error, generateWeeklySummary } =
        ...
        <div className="grid grid-cols-1 gap-2 text-sm">
          {trends.sleep_trend && (
            <div className="flex justify-between">
              ...
              <span>
                {trends.sleep_trend.consistency} ({trends.sleep_trend.average}h
                avg)
              </span>
            </div>
          )}
          ...
          {trends.energy_trend && (
            <div className="flex justify-between">
              ...
              <span
                className={`${
                  trends.energy_trend.direction === 'improving'
                    ? 'text-green-400'
                    : trends.energy_trend.direction === 'declining'
                      ? 'text-red-400'
                      : 'text-yellow-400'
                }`}
              >
                {trends.energy_trend.direction} ({trends.energy_trend.average}
                /10 avg)
              </span>
            </div>
          )}
          ...
          {trends.mood_trend && (
            <div className="flex justify-between">
              ...
              <span
                className={`${
                  trends.mood_trend.direction === 'improving'
                    ? 'text-green-400'
                    : trends.mood_trend.direction === 'declining'
                      ? 'text-red-400'
                      : 'text-yellow-400'
                }`}
              >
                {trends.mood_trend.direction} ({trends.mood_trend.average}/10
                avg)
              </span>
            </div>
          )}
          ...
          {trends.workout_consistency && (
            <div className="flex justify-between">
              ...
              <span
                className={`${
                  trends.workout_consistency.rating === 'excellent'
                    ? 'text-green-400'
                    : trends.workout_consistency.rating === 'good'
                      ? 'text-yellow-400'
                      : 'text-red-400'
                }`}
              >
                {trends.workout_consistency.rating} (
                {trends.workout_consistency.days}/
                {trends.workout_consistency.total_days} days)
              </span>
            </div>
          )}
        </div>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-muted mb-4">
              No weekly summary available for this week.
            </p>
            <Button
              onClick={() => generateWeeklySummary(weekStart)}
              disabled={loading}
