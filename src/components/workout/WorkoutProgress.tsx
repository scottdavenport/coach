...
export function WorkoutProgress({
  ...
  const loadProgressData = async () => {
    ...
    try {
      ...
      const progressResponse = await fetch(
        `/api/workouts/progress?timeRange=${selectedTimeRange}`
      );
  ...
  const calculateStats = () => {
    ...
    const completedWorkouts = recentWorkouts.filter(
      w => w.completion_status === 'completed'
    ).length;
    const totalDuration = recentWorkouts.reduce(
      (sum, w) => sum + (w.total_duration || 0),
      0
    );
    const averageIntensity =
      recentWorkouts.reduce((sum, w) => sum + (w.perceived_exertion || 0), 0) /
      Math.max(totalWorkouts, 1);

    const personalRecords = recentProgress.filter(
      p => p.metric_type === 'personal_record'
    ).length;
    const strengthGains = recentProgress.filter(
      p => p.metric_type === 'max_weight'
    ).length;
    const enduranceGains = recentProgress.filter(
      p => p.metric_type === 'max_duration'
    ).length;
  ...
  const getRecentPersonalRecords = () => {
    return progress
      .filter(
        p =>
          p.metric_type === 'personal_record' ||
          p.metric_type === 'max_weight' ||
          p.metric_type === 'max_reps'
      )
      .sort(
        (a, b) =>
          ...
          {USE_MOCK_DATA ? (
              ...
              {stats.completionRate >= 80 && (
                <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <p className="text-sm text-green-800">
                    Excellent workout consistency! You're completing{' '}
                    {Math.round(stats.completionRate)}% of your planned
                    workouts.
                  </p>
                </div>
              )}
              ...
              {stats.completionRate < 50 && (
                <div className="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  <p className="text-sm text-yellow-800">
                    Consider reducing workout intensity or duration to improve
                    completion rate.
                  </p>
                </div>
              )}
              ...
              {stats.personalRecords > 0 && (
                <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-sm text-blue-800">
                    Great progress! You've set {stats.personalRecords} personal
              ...
              {stats.averageIntensity > 7 && (
                <div className="flex items-center gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  ...
                  <p className="text-sm text-orange-800">
                    High intensity workouts detected. Make sure to include
              ...
              {stats.totalWorkouts === 0 && (
                <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                  ...
                  <p className="text-sm text-gray-800">
                    No workouts recorded in this time period. Start with small,
                    achievable goals!
                  </p>
                </div>
              )}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted max-w-md mx-auto mb-4">
                Complete some workouts to receive personalized progress insights
                and recommendations.
              </p>
            </div>
          )}
